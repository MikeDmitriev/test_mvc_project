...
Так же заведены REST’ы, позволяющие по методу GET:
* получить список всех заявок (ответ - JSON);
* получить список всех заявок владельца по его паспортным данным (серия и номер) (ответ - JSON);
* получить список всех счетов владельца (ответ - JSON);
* получить список всех банковских карт владельца (ответ - JSON);
* получить список всех счетов в базе данных (ответ - JSON);
* получить список всех владельцев в базе данных (ответ - JSON);
* получить список всех идентификационных номеров владельцев; (ответ - JSON)
* получить информацию о конкретной карте по ее номеру (ответ - JSON);
